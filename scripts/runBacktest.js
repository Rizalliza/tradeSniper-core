...
if (s.perSymbol.length) {
    ...
    for (const ps of s.perSymbol) {
        console.log(`  ${ps.symbol.padEnd(6)} setups=${String(ps.setups).padEnd(3)} taken=${String(ps.taken).padEnd(3)} win=${ps.win_rate.toFixed(1).padStart(5)}%  PnL=$${ps.pnl.toFixed(2).padStart(8)}`);
    }
}
...
if (trades.length) {
    ...
    for (const t of trades.slice(-10)) {
        const sign = t.pnl >= 0 ? '+' : '';
        console.log(`  ${t.date} ${t.symbol.padEnd(4)} ${t.bias.padEnd(4)} ${t.entry_price.toFixed(2)} → ${t.exit_price.toFixed(2)} (${t.exit_reason}) ${sign}$${t.pnl.toFixed(2)}`);
    }
}