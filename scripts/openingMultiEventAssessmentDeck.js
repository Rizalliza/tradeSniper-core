#!/usr/bin/env node
/**
 * Cleaner assessment deck for multi-event scanner output.
 * Plots only selected candidates and compact touch/cross labels by default.
 */
...
function parseArgs() {
    ...
    const opts = {
        ...
        assessment: 'data/opening-multi-event-assessment-2026-09-09_2026-09-15.json',
        out: 'webapp/opening-multi-event-assessment.html',
    };
    for (let i = 0; i < args.length; i++) {
        switch (args[i]) {
            case '--pressure': opts.pressure = args[++i]; break;
            case '--assessment': opts.assessment = args[++i]; break;
...
function chart({ session, pack }) {
    ...
    const selected = session.selectedCandidates || [];
    const width = 1120;
    const height = 410;
    ...
    for (const c of selected) {
        for (const value of [c.entryCandidateClose, c.fill?.fillPrice, c.trade?.exitPrice]) {
    ...
    for (const a of compactAnnotations(session.annotations || [])) {
        const color = a.side === 'BUY' ? '#54d990' : '#ff6b6b';
        parts.push(smallLabel(x(a.barIndex), y(a.levelValue), color, a.label));
    }
    for (const c of selected) {
        const color = c.direction === 'BUY' ? '#54d990' : '#ff6b6b';
        const candidateIndex = findBarIndex(bars, c.entryCandidateTime);
        parts.push(bigMarker(x(candidateIndex), y(c.entryCandidateClose), color, `${c.candidateId} ${c.direction}`));
        if (c.fill?.status === 'FILLED') {
            const fillIndex = findBarIndex(bars, c.fill.fillTime);
            parts.push(bigMarker(x(fillIndex), y(c.fill.fillPrice), color, `${c.candidateId} FILL`));
        }
        if (c.trade) {
            const exitIndex = findBarIndex(bars, c.trade.exitTime);
            parts.push(bigMarker(x(exitIndex), y(c.trade.exitPrice), color, `${c.candidateId} EXIT ${shortExit(c.trade.exitReason)}`));
        }
    }
...
function compactAnnotations(annotations) {
    return annotations.filter(a => a.ordinal <= 3 || a.kind === 'CROSS');
}

function smallLabel(x, y, color, label) {
    return `<g opacity="0.88"><circle cx="${x}" cy="${y}" r="3" fill="${color}"/><text x="${x + 5}" y="${y + 4}" fill="${color}" font-size="9" font-weight="700">${esc(label)}</text></g>`;
}

function bigMarker(x, y, color, label) {
    return `<g><circle cx="${x}" cy="${y}" r="6" fill="${color}" stroke="#101419" stroke-width="2"/><text x="${x + 9}" y="${y - 8}" fill="${color}" font-size="11" font-weight="800">${esc(label)}</text></g>`;
}

function card({ session, pack }) {
    const selected = session.selectedStats || {};
    const raw = session.allStats || {};
    return `
      <article class="card">
        <header>
          <div>
            <h2>${esc(session.symbol)} ${esc(session.date)}</h2>
            <p>Bias ${esc(session.bias.direction)} ${pct(session.bias.returnPct)} · selected ${selected.trades || 0} trades W/L ${selected.wins || 0}/${selected.losses || 0} total ${pct(selected.totalPnlPct)} · raw filled total ${pct(raw.totalPnlPct)}</p>
          </div>
          <div class="badge">${esc((session.actionBrief || []).at(-1) || '')}</div>
        </header>
        ${chart({ session, pack })}
        <section class="brief">
          ${(session.actionBrief || []).map(line => `<p>${esc(line)}</p>`).join('')}
        </section>
        <section>
          <h3>Selected Candidates</h3>
          ${candidateTable(session.selectedCandidates || [])}
        </section>
        <section>
          <h3>Touch / Cross Summary</h3>
          <p>${esc(summaryText(session.annotationSummary))}</p>
        </section>
      </article>`;
}
...
function candidateTable(candidates) {
    const rows = candidates.map(c => `
      <tr>
        ...
        <td>${esc(c.biasAlignment)} x${num(c.biasMultiplier, 1)}</td>
        <td>${num(c.preOutcomeScore, 1)}</td>
        <td>${esc(c.trade ? `${shortExit(c.trade.exitReason)} ${pct(c.trade.pnlPct)}` : c.lifecycle)}</td>
      </tr>`).join('');
    return `<table><thead><tr><th>ID</th><th>Time</th><th>Side</th><th>Type</th><th>Sep bps</th><th>Bias</th><th>Score</th><th>Result</th></tr></thead><tbody>${rows}</tbody></table>`;
}

function html({ assessment, packMap }) {
    const cards = (assessment.sessions || [])
        .map(session => {
...
<head>
  ...
  <title>Opening Multi-Event Assessment</title>
  <style>
    ...
    .sub,p{color:#91a1b5;margin:0 0 10px;font-size:13px;line-height:1.4}
    ...
    .badge{font-size:11px;background:#1b2533;border:1px solid #344357;border-radius:8px;padding:8px 10px;color:#d8e2ef;max-width:45%;line-height:1.35}
    ...
    @media(max-width:860px){main{padding:12px}.summary{grid-template-columns:repeat(2,1fr)}header{display:block}.badge{max-width:none;margin-top:8px}}
  </style>
</head>
<body>
<main>
  <h1>Opening Multi-Event Assessment</h1>
  <p class="sub">${esc(assessment.note)} · generated ${esc(assessment.generated_at)}</p>
  <section class="summary">
    ${metric('sessions', s.sessions ?? '-')}
    ${metric('raw candidates', s.rawCandidates ?? '-')}
    ${metric('selected', s.selectedCandidates ?? '-')}
    ${metric('selected WR', pct(s.selectedStats?.winRate))}
    ${metric('selected total', pct(s.selectedStats?.totalPnlPct))}
    ${metric('raw filled total', pct(s.rawFilledStats?.totalPnlPct))}
  </section>
  ${cards}
</main>
</body>
...
function summaryText(summary = {}) {
    return `total ${summary.total || 0}; side ${pairs(summary.bySide)}; kind ${pairs(summary.byKind)}; level ${pairs(summary.byLevel)}`;
}
...
async function main() {
    ...
    const assessment = JSON.parse(await fs.readFile(opts.assessment, 'utf8'));
    const output = html({ assessment, packMap: buildPackMap(pressure) });
